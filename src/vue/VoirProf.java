...
public class VoirProf extends javax.swing.JPanel {
    ...
    private void initComponents(int a) {
        ...
        try{
            Personne prof=ecole.seekerPersonne(a);
            String info;
            info = prof.getprenom()+"  "+prof.getnom()+"      "+annee;
            jLabel1.setText(info);
            ArrayList<Enseignement> enseignements = null;
            ecole.getmineEns(prof,enseignements);
            for(Enseignement elem: enseignements){
                String classe=ecole.seekerClasse(elem.getidClasse()).getnom();
                String discipline=ecole.seekerDiscipline(elem.getidDiscipline()).getnom();
                double moyenne= ecole.MoyenneProf(elem.getid());
    ...
    private String[] columnNames = {"Classe","Discipline"};
